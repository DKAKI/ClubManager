...
public class TreasuryDialog extends JDialog {
	...
	private TreasuryAppUser thetreasuryAppUser;
	public TreasuryDialog(TreasuryAppUser treasuryAppUser,
			TreasuryDAO TreasuryDAO2, Treasury thePreviousTreasury, boolean theUpdateMode) {
		this();
		treasuryDAO = TreasuryDAO2;
		thetreasuryAppUser = treasuryAppUser;
	...
	public TreasuryDialog(TreasuryAppUser thetreasuryAppUser, TreasuryDAO treasuryDAO2) {
		// TODO Auto-generated constructor stub
		this(thetreasuryAppUser, treasuryDAO2, null, false);
	}
	...
	/*
	 * else if (!Integer.class.isInstance(yearTextField.getText()))
			JOptionPane.showMessageDialog(treasuryApp,"Enter a valid year  ", "Error",JOptionPane.ERROR_MESSAGE);
		else if (!Integer.class.isInstance(sumTextField.getText()))
			JOptionPane.showMessageDialog(treasuryApp,"Enter a valid Sum  ", "Error",JOptionPane.ERROR_MESSAGE); 
	*/
	...
	protected void saveTreasury() {
		...
		catch ( NumberFormatException JKLdH) {
			JOptionPane.showMessageDialog(treasuryApp,"check the value of the year or/and the sum ", "Error",JOptionPane.ERROR_MESSAGE);
		}
		
		if (yearTextField.getText().equals("") || sumTextField.getText().equals("")){
			 year=0;
			 sum=0;
		} 
		else {
		...
		try {
			...
			if (updateMode) {
					if (year==0 || sum==0 ||  stateOfTreasury.equals("") ||  responsibleTreasury.equals("")) {
					
						JOptionPane.showMessageDialog(treasuryApp,"You should fill out all the fields ", "Error",JOptionPane.ERROR_MESSAGE);
					return;}
					else
			...
			// refresh gui list
			if (treasuryApp.frame.isVisible())
			treasuryApp.refreshTreasuryView();
			else  {
			thetreasuryAppUser.refreshTreasuryView();
			}
